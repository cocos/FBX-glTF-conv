...
struct ConvertOptionBindingTrait<
    &bee::ConvertOptions::animation_position_error_multipler> {
  constexpr static auto name = "animation-position-error-multipler";
  constexpr static auto description = "Animation position error multipler.";
  constexpr static auto default_value = "1e-5";
};

template <>
struct ConvertOptionBindingTrait<
    &bee::ConvertOptions::animation_scale_error_multipler> {
  constexpr static auto name = "animation-scale-error-multipler";
  constexpr static auto description = "Animation scale error multipler.";
  constexpr static auto default_value = "1e-5";
};
...
struct convert_option_binding_helper<optionMemberPtr> {
  constexpr static auto add_cxx_option(cxxopts::Options &cxx_options_,
    ...
    bee::ConvertOptions defaultOptions;
    const auto defaultOptionValue = defaultOptions.*optionMemberPtr;
  ...
  fetch_convert_option(const cxxopts::ParseResult &cxxopts_parse_result_,
    ...
    if (cxxopts_parse_result_.count(Trait::name)) {
      convert_options_.*optionMemberPtr =
          cxxopts_parse_result_[Trait::name].as<OptionType>();
    }
  }
};
...
std::optional<CliArgs> readCliArgs(std::span<std::string_view> args_) {
  ...
  operator()<&bee::ConvertOptions::animation_position_error_multipler>();

  add_cxx_option.template
  operator()<&bee::ConvertOptions::animation_scale_error_multipler>();
